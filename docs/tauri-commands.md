...
| `generate_apply_cv` | `{ opportunity_id }` | `GenerateApplyCvResult` | Export pack if needed, then spawn `generate-apply-cv` (`kanithanj.cv` / `bun` / `pnpm`). Requires the CV maker script. Writes the CV PDF and, when `cover-letter.md` has letter text, a cover-letter PDF beside it. An empty letter writes no letter PDF. **No master CV write.** |
