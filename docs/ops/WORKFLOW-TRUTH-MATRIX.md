...
Inventoried only. Typst and PDF-UA are not implemented here. Rows 1 and 2 were false-green apply risks: the pack can look ready to submit while a later look pull restores a two-column layout or a long token overflows the page box, breaking the ATS single-column claim. PR #73 landed the pull guard and soft-wrap for those two cases.

| # | Workflow | Entry | Status | E2E smoke | Evidence | Verdict |
|---|----------|-------|--------|-----------|----------|---------|
| 1 | `scripts/pull-cv-renderer.sh` overwrite | Operator script. Default ref is devprofile `main`. Allowlist copies `vendor/kanithanj-cv/src/components/cv-document.tsx` with `cp`. | **false-green risk (two-column after look pull)** | `bash scripts/test-pull-cv-renderer.sh` (passed this round). It writes a fixture over `cv-document.tsx` and only refuses paths outside the allowlist. | Comment in the script says a pull replaces the apply layout and that a two-column page fails positional ATS extraction, then the script still overwrites the file. `#71` single-column lives in the vendored `cv-document.tsx` (`Font.registerHyphenationCallback`, one column). `scripts/test-cv-sot.sh` checks the allowlist and does not refuse a two-column body. Hillclimb: re-smoke `vendor/kanithanj-cv` `bun scripts/ats-pdf-smoke.tsx` after every pull. | **complete** (guard). Not this change. |
| 2 | Long-token overflow | Apply PDF text in `vendor/kanithanj-cv/src/components/cv-document.tsx`. | **half** (false-green: smoke vs page box) | `vendor/kanithanj-cv/scripts/ats-pdf-smoke.tsx` fails if the hyphenator splits `Orientation` or `UNBROKEN_SENTINEL_ORIENTATION_TOKEN`. It does not measure whether a long token stays inside the page box. | `Font.registerHyphenationCallback((word) => [word])` keeps words unbroken. `listText` / tools lines have no clamp or wrap for a token wider than the line, so a URL or sentinel can paint past the margin. The smoke treats "not hyphenated" as success. | **complete** (clamp or wrap the token; do not put hyphenation back and call that ATS-safe). |
