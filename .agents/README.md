...
- **The collab-finder app itself**: Exposes hunter capabilities as MCP tools + publishes its own root `SKILL.md` (so external agents can call "search X opportunities with my profile + CV", "generate prep pack with guards", etc.).

## Core + Project Skills

See root `AGENTS.md` for the full index (hunter-reactor, x-agent-resources, cv-promote-guard, tauri-agentic, plus portable ai-optimization, fusion-sage, bdd-strategizer, agent-orchestrator, git-worktrees, etc.).
...
- `x-hunter.mdc` — X resources (skill.md, llms, MCP, xurl) + query control.
...
- Publish matching `SKILL.md` + MCP server so the hunter is agent-composable.
...
- Surplus after major tasks (cheaper future iterations of the hunter).
...
- The app under development is also the tool that accelerates future dev (agents use the hunter to find collabs for the project itself, or prep materials).
